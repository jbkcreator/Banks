...
- **E-D3 — Slack approval: Block Kit buttons via Socket Mode are the PRIMARY mechanism (BUILT + proven live 2026-08-05); emoji reactions kept as fallback.** Supersedes A-D8's reaction-primary stance (reactions demoted to belt-and-suspenders fallback). Delivered: `banks/approval.py` (pure `render_draft_blocks` + `apply_action` driving the `decision_packets` two-step — Approve→`answered_at`, Mark sent→`completed_at`, Reject/Revise→no DB, no send; financial detail withheld from Slack), `banks/socket_listener.py` (always-on outbound WebSocket via `xapp-` token, no public server), +6 tests (60 total). Verified end-to-end against the test workspace: click → WebSocket → state change → in-place message update. Trade-off accepted: buttons need the always-on listener process (Phase-G box in prod); if it's down, reaction fallback still catches approvals. **Original grill note (still true):** Correction to the earlier "buttons need a public server" claim: **Socket Mode** receives interactivity over a persistent outbound WebSocket (uses the `xapp-` app-level token already provisioned), so buttons work behind a firewall with no public endpoint. Real trade-off is stateless-poll vs always-on listener process (the latter needs a long-lived process, which the Phase-G box provides anyway). **Block Kit blocks for display** are usable now regardless (pure outbound). Default stays reactions+polling for the stateless 80%; buttons+Socket Mode selectable behind the same ChatPort seam without a domain change.
